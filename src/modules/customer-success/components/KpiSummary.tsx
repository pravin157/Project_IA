...
import { PortfolioSummary, HealthBucketFilter } from '@/types/dashboard';
import {
  Users,
  HeartPulse,
  AlertTriangle,
...
export const KpiSummary: React.FC<KpiSummaryProps> = ({
  ...
  const cards = [
    ...
    },
    {
      ...
      icon: HeartPulse,
      accent: avgHealthScore >= 70 ? '#059669' : avgHealthScore >= 40 ? '#d97706' : '#e11d48',
      accentBg: avgHealthScore >= 70 ? 'rgba(5,150,105,0.1)' : avgHealthScore >= 40 ? 'rgba(217,119,6,0.1)' : 'rgba(225,29,72,0.1)',
    },
    {
      key: 'attention',
      label: 'Need Focus',
    ...
    },
    {
      ...
      icon: Bell,
      accent: totalCriticalAlerts > 0 ? '#e11d48' : '#7c3aed',
      accentBg: totalCriticalAlerts > 0 ? 'rgba(225,29,72,0.1)' : 'rgba(124,58,237,0.1)',
      ...
      {cards.map((card) => {
        ...
        return (
          <div
            ...
            className={`relative rounded-2xl p-4 border transition-all duration-200 animate-fade-in overflow-hidden ${card.onClick ? 'cursor-pointer hover:scale-[1.02] hover:shadow-md' : ''}`}
            style={{
              background: '#FFFFFF',
              borderColor: '#E5E7EB',
          ...
          >
            {/* Glow orb */}
            <div className="absolute -top-4 -right-4 w-20 h-20 rounded-full opacity-10 blur-2xl pointer-events-none"
              style={{ background: card.accent }} />

            <div className="flex items-center justify-between mb-3">
              <span className="text-[11px] font-bold text-[#000000]/50 uppercase tracking-wider">{card.label}</span>
            ...
            <div className="flex items-baseline gap-1.5 mb-1">
              ...
              <span className="text-[11px] text-[#000000]/60">{card.suffix}</span>
            </div>
            <div className="text-[11px] text-[#000000]/60 truncate">{card.hint}</div>
          </div>
        );
      })}
    </div>
